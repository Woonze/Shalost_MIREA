...
- Окно «Контроль присутствия» в MTS Link распознаётся на экране раз в 2 секунды; кнопка «ПОДТВЕРЖДАЮ» нажимается автоматически.
- После успешного подтверждения действует настраиваемая пауза, по умолчанию 10 минут.
- Понятный журнал остаётся в окне, полный технический журнал — в pulseqr.log рядом с EXE.
...
В интерфейсе выводятся только рабочие события UI, AUTH, SCAN и PULSE. Полная диагностика записывается в pulseqr.log рядом с программой. Это позволяет не засорять экран техническими ошибками, но сохраняет данные для отладки.
